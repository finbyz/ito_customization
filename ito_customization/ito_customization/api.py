...
def save_bulk_student_list():
	try:
		...
		try:
			...
			target_class = str(selected_class).strip()
			clean_class = str(selected_class).lower().replace("class", "").strip()
			if frappe.db.exists("Class", target_class):
				pass
			elif frappe.db.exists("Class", clean_class):
				target_class = clean_class
			else:
				found_c = frappe.db.get_value("Class", {"class": clean_class}, "name")
				if found_c:
					target_class = found_c
			...
			for student in students:
				...
				for sub_key, sub_val in subj_data.items():
			...
			return {"success": True, "name": bsl.name, "roll_numbers": saved_roll_numbers}
	...
	try:
		...
		try:
			...
			if bsl_name:
				...
				if selected_class:
					remaining_rows = [
						row for row in (bsl.little_champ_list or [])
						if not is_same_class(row.get("class"), selected_class)
					]
					bsl.set("little_champ_list", remaining_rows)
				else:
					bsl.set("little_champ_list", [])
			else:
				bsl = frappe.new_doc("Bulk Student List")
				bsl.customer = customer_name
				bsl.academic_year = academic_year
			...
			ito_code = (
				...
				or school_info.get("lc_school_code")
			...
			for student in students:
				...
				for sub_key, sub_val in subj_data.items():
					...
					# Append to BSL little_champ_list child table
				...
				try:
					...
					for proc in items_to_process:
						...
						try:
							...
							frappe.log_error(frappe.get_traceback(), "Registered Students Sync Error (Little Champ)")
					...
					for r in existing_records:
						if r["name"] not in matched_existing_names and not r.get("not_registered"):
							try:
								...
								frappe.log_error(frappe.get_traceback(), "Registered Students Retire Error (Little Champ)")

				except Exception:
					frappe.log_error(frappe.get_traceback(), "Registered Students Sync Block Error (Little Champ)")
			...
			if selected_class:
				frappe.cache().set_value(f"little_champ_bulk_draft_{customer_name}_{academic_year}_{selected_class}", payload)
				frappe.cache().set_value(f"little_champ_bulk_draft_{customer_name}_{raw_year}_{selected_class}", payload)
			# Remove un-scoped cache so it doesn't mask other saved classes
			frappe.cache().delete_value(f"little_champ_bulk_draft_{customer_name}_{academic_year}")
			frappe.cache().delete_value(f"little_champ_bulk_draft_{customer_name}_{raw_year}")
...
@frappe.whitelist()
def get_little_champ_bulk(academic_year=None, selected_class=None):
	try:
		customer_name = frappe.db.get_value("Portal User", {"user": frappe.session.user}, "parent")
		if not customer_name:
			return {"success": False, "message": "Customer not found", "data": None}
		...
		saved_data = None
		if selected_class:
			saved_data = frappe.cache().get_value(f"little_champ_bulk_draft_{customer_name}_{academic_year}_{selected_class}")
			if not saved_data:
				saved_data = frappe.cache().get_value(f"little_champ_bulk_draft_{customer_name}_{raw_year}_{selected_class}")
		else:
			saved_data = frappe.cache().get_value(f"little_champ_bulk_draft_{customer_name}_{academic_year}")
			if not saved_data:
				saved_data = frappe.cache().get_value(f"little_champ_bulk_draft_{customer_name}_{raw_year}")
		...
		if bsl_name:
			...
			student_rows = bsl.little_champ_list or bsl.student_list or []
			students_map = {}
			...
			if students_map:
				...
				if frappe.db.exists("DocType", "Registered Students"):
					try:
						...
						frappe.log_error(frappe.get_traceback(), "Get Little Champ Bulk - Roll No Enrich Error")

				school_doc = frappe.get_doc("Customer", customer_name)
				lc_code = school_doc.custom_school_code or school_doc.custom_lc_school_code or school_doc.custom_ito_school_code or ""
				return {
					"success": True,
					"data": {
						"school_info": {
							"school_name": school_doc.customer_name or "",
							"lc_school_code": lc_code,
							"selected_class": "",
							"academic_year": bsl.academic_year or academic_year,
						},
						"students": students_list
		...
		frappe.log_error(frappe.get_traceback(), "Get Little Champ Bulk Error")
		return {"success": False, "message": str(e), "data": None}
